...
///
/// Holds the emit-time [`Effect`]. The captured value (with resolved data) is materialized
/// only when [`take_effects`] runs the `capture()` step inside its turbo-tasks task context.
/// As a result, the `EffectInstance` cell value itself is small — it does not carry
/// pre-resolved `ReadRef`s, so it does not pin upstream cell data.
...
pub async fn take_effects(source: impl CollectiblesSource) -> Result<Effects> {
    ...
    let effect_refs: Vec<ReadRef<EffectInstance>> = source
