...
| Custom server compatibility               | `next.ts` wrapper and public methods                                            | `getRequestHandler()`, `render()`, `renderToHTML()`, `renderError()`, `render404()`, and `revalidate()` must remain compatible.                        |
...
- i18n: Locale detection redirects, domain locales, default-locale internal
  prefixing, API route locale stripping, and locale metadata must remain byte
...
- HMR upgrades: Dev HMR path handling depends on `basePath`/`assetPrefix` and
  should remain in `router-server.ts` until routing equivalence is proven.
